...
def run_clustering(df, target_col, cost_col, additional_cols, date_col, geo_col, exclude_dates, weight_str):
    ...
    try:
        ...
        if weight_str and weight_str.strip() != "":
            try:
                raw_weight_map = ast.literal_eval(weight_str)
                # Translate 'weight_' back to 'avg_' so downstream logic survives
                weight_map = {
                    k.replace('weight_', 'avg_', 1) if k.startswith('weight_') else k: v 
...
def generate_empirical_causal_weights(sem_df, target_col='revenue', alpha=0.10):
    ...
    weight_map = {f'weight_{target_col}': 1.0} # <--- CHANGED FROM 'avg_'
    ...
    for node, val in log_effects.items():
        ...
        weight_map[f'weight_{node}'] = round(scaled_val, 4) # <--- CHANGED FROM 'avg_'
...
def prep_design_tab(df, sem_target, best_lever, sem_weights_str, dropped_cols):
    ...
    if sem_weights_str and sem_weights_str.strip():
        try:
            ...
            # CHANGED: Strip 'weight_' instead of 'avg_' to map back to original columns
            cluster_features = [k.replace("weight_", "") for k in weight_map.keys()]
